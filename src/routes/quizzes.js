...
async function attachQuizCounts(quizzes) {
  ...
  const [questionRes, attemptRes] = await Promise.all([
    supabaseAdmin.from("questions").select("quiz_id").in("quiz_id", ids),
    supabaseAdmin.from("attempts").select("quiz_id").in("quiz_id", ids),
  ]);
...
router.delete("/:id", requireAuth, requireAdmin, async (req, res) => {
  ...
  // Fetch all questions (with image URLs) before cascaded delete wipes them
  ...
  // Clean up all question/option images from storage after successful DB delete
  if (questions?.length) await deleteQuestionImages(questions);
