...
router.post('/quizzes/:quizId/questions', requireAuth, requireAdmin, async (req, res) => {
  const { question_text, question_image_url, options, correct_option_index, explanation } =
    req.body

  if (!question_text || !Array.isArray(options) || options.length < 2 || correct_option_index == null) {
    return res.status(400).json({ error: 'Missing or invalid question fields' })
  }
...
router.post('/quizzes/:quizId/questions/import', requireAuth, requireAdmin, async (req, res) => {
  const { questions } = req.body

  if (!Array.isArray(questions) || questions.length === 0) {
    return res.status(400).json({ error: 'Expected a non-empty "questions" array' })
  }
  ...
  const invalid = rows.find(
    (r) => !r.question_text || !Array.isArray(r.options) || r.options.length < 2 || r.correct_option_index == null
  )
  if (invalid) {
